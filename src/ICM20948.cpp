...
ICM_20948_AGMT_t ICM_20948::getAGMT() {
  status = ICM_20948_get_agmt(&_device, &agmt);
  return agmt;
}

float ICM_20948::magX() {  return getMagUT(agmt.mag.axes.x);  }
float ICM_20948::magY() {  return getMagUT(agmt.mag.axes.y);  }
float ICM_20948::magZ() {  return getMagUT(agmt.mag.axes.z);  }
float ICM_20948::getMagUT(int16_t axis_val) {  return (((float)axis_val) * 0.15);  }

float ICM_20948::accX() {  return getAccMG(agmt.acc.axes.x);  }
float ICM_20948::accY() {  return getAccMG(agmt.acc.axes.y);  }
float ICM_20948::accZ() {  return getAccMG(agmt.acc.axes.z);  }

float ICM_20948::gyrX() {  return getGyrDPS(agmt.gyr.axes.x);  }
float ICM_20948::gyrY() {  return getGyrDPS(agmt.gyr.axes.y);  }
float ICM_20948::gyrZ() {  return getGyrDPS(agmt.gyr.axes.z);  }


float ICM_20948::getAccMG(int16_t axis_val) {
  switch (agmt.fss.a) {
    case 0:    return (((float)axis_val) / 16.384);
    case 1:    return (((float)axis_val) / 8.192);
    case 2:    return (((float)axis_val) / 4.096);
    case 3:    return (((float)axis_val) / 2.048);
    default:   return 0;
  }
}
...
float ICM_20948::getGyrDPS(int16_t axis_val) {
  switch (agmt.fss.g) {
    case 0:   return (((float)axis_val) / 131);
    case 1:   return (((float)axis_val) / 65.5);
    case 2:   return (((float)axis_val) / 32.8);
    case 3:   return (((float)axis_val) / 16.4);
    default:  return 0;
  }
}


float ICM_20948::temp() {  return getTempC(agmt.tmp.val);   }
...
const char *ICM_20948::statusString(ICM_20948_Status_e stat) {
  ICM_20948_Status_e val = (stat == ICM_20948_Stat_NUM) ? status : stat;
  switch (val) {
    case ICM_20948_Stat_Ok:                   return "All is well.";
    ...
    case ICM_20948_Stat_NoData:               return "Data Underflow";
    case ICM_20948_Stat_SensorNotSupported:   return "Sensor Not Supported";
    default:                                  return "Unknown Status";
  }
  return "None";   // WHY???? This cannot possibly be returned.
}
...
ICM_20948_Status_e ICM_20948::swReset() {
...
ICM_20948_Status_e ICM_20948::clearInterrupts() {
  ICM_20948_INT_STATUS_t int_stat;
  ICM_20948_INT_STATUS_1_t int_stat_1;

  status = ICM_20948_set_bank(&_device, 0);   // read to clear interrupts
...
ICM_20948_Status_e ICM_20948::startupDefault() {
...
ICM_20948_Status_e ICM_20948_I2C::begin(TwoWire &wirePort, bool ad0val, uint8_t ad0pin) {
  ...
  if (_ad0 != ICM_20948_ARD_UNUSED_PIN) {    // Set pinmodes
    pinMode(_ad0, OUTPUT);
  }

  if (_ad0 != ICM_20948_ARD_UNUSED_PIN) {
    digitalWrite(_ad0, _ad0val);   // Set pins to default positions
  }
  ...
  _device._serif = &_serif;   // Link the serif
  status = startupDefault();  // Perform default startup
  return status;
}
...
ICM_20948_Status_e ICM_20948_SPI::begin(uint8_t csPin, SPIClass &spiPort, uint32_t SPIFreq) {
  if (SPIFreq > 7000000) SPIFreq = 7000000;
...
ICM_20948_Status_e ICM_20948_write_I2C(uint8_t reg, uint8_t *data, uint32_t len, void *user) {
  ICM_20948_Status_e ret = ICM_20948_Stat_ParamErr;
  if (user != NULL) {
    ...
    if (_i2c != NULL) {
      ret = ICM_20948_Stat_Err;  // TODO: Need enum for bus error.
